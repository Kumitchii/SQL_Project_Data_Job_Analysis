...
WITH average_country_salary AS (
    SELECT
        job_country,
...
SELECT
    ...
    salary_year_avg,
    CASE
        WHEN salary_year_avg > avg_salary THEN 'Above Average'
        ELSE 'Below Average'
    END AS salary_category,
    EXTRACT(MONTH FROM job_posted_date) AS postings_month
FROM job_postings_fact
    INNER JOIN average_country_salary ON job_postings_fact.job_country = average_country_salary.job_country
